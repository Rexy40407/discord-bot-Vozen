...
| 1.4 | Consent-first for anything acting on/about a user (recording) | ✅ | Voice clone: on-screen Allow button records the target's consent (`voice.ts`, `store/voiceClone.ts`). STT: per-speaker consent gate (`transcriptionSession.ts` `hasConsent`), un-deafen only during a session, audio never persisted. |
| 1.5 | Every stored user datum disclosed AND has a user deletion path | ✅ | `PRIVACY.md §Your rights` table maps each datum to a command; `/privacy erase` wipes everything by user ID (`handlers/privacy.ts` + `store/dataLifecycle.eraseUser`); clone-derived audio also purged (`purgeCloneDerivedAudio`, audit fix). |
...
| 2.2 | Lawful basis per purpose (Art. 6) | ✅ | `site/privacy.html §3`: 6(1)(b) service, (f) legitimate interests, (a) consent for clone/STT, (c)/(f) financial records. |
...
| 2.5 | Consent is explicit + withdrawable (Art. 7) for special-category-adjacent data (voice) | ✅ | Clone consent = Allow button; STT consent per speaker; withdrawal: `/voice clone delete`, `/transcribe revoke`. |
...
| 3.2 | CCPA/CPRA — disclosure + deletion + no-sale | ✅ | No sale of personal information; disclosure in `PRIVACY.md`; deletion via `/privacy erase`. Given self-host scale, most CCPA business thresholds do not apply, but the mechanisms exist regardless. |
