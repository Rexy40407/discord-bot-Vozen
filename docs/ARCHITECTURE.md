...
| `premium/kofi.ts` + `kofiWebhook.ts` | Webhook do Ko-fi (opção A): parse do payload + verificação de token + extração do Discord ID da mensagem + mapa produto→grant (premium/plus × mensal/anual) — puro; o servidor HTTP fino aplica via `grant*` (source `kofi`). Inerte sem `KOFI_WEBHOOK_TOKEN`. **Dois caminhos de produto:** as *memberships* (mensais) são lidas por palavras-chave no `tier_name`; os *shop items* (passes ANUAIS) NÃO trazem o nome do produto no payload — só o `direct_link_code` (o `variation_name` só existe em produtos FÍSICOS) — por isso são resolvidos por `parseShopMap(KOFI_SHOP_MAP)` (`codigo:plano:dias[:licencas]`), verificado ANTES das palavras-chave. Sem o mapa, uma compra de loja é ignorada em silêncio; entradas malformadas são saltadas (um typo na env não derruba o webhook). Compra → `/premium activate`. Vendas manuais/recuperação: `/vozengrant` (OWNER-ONLY, registado só na `OWNER_GUILD_ID`, gate pelo dono real da application). O MESMO servidor HTTP também roteia a API do Painel Premium (ver `statusApi.ts`) quando ativa. | `store/premium`, `premium/statusApi` |
