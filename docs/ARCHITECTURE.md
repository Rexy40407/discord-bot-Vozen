...
| `tts/gcloud.ts` | `GCloudEngine` (**Google HD**, perk Premium — API oficial Google Cloud TTS Standard, LINEAR16 22050Hz). `bcp47OfModel` deriva o `languageCode` do id do modelo. **CHOKEPOINT de custo:** conta chars SÓ em cache-miss (chamada real) e aplica os tetos ANTES de gastar $ — `enforceBudget` lança `GcloudBudgetError` (→ gTTS) sem orçamento (fail-safe), acima de `maxChars`, com o pool mensal esgotado, ou com o backstop diário estourado; `accountUsage` debita depois do sucesso. `SEM GOOGLE_TTS_API_KEY nem é construído (o caminho 'gcloud' = gTTS). Métricas `gcloud_synths/chars/fallbacks`. Ligado em `createPerUserEngine` num `RouterEngine([gcloud→gTTS])`. | `cache`, `gcloudUsage`, `metrics`, `deCaps`, `wavConcat` |
| `tts/resolveEngine.ts` | `resolveUserEngine(db, guild, user, storedEngine, now)`: resolvedor **partilhado** ligado a TODOS os call sites que copiam `userVoice.engine` para o pedido (messageHandler, /joke, /laugh, /rizz, micro-fun, /voice preview). Despromove `gcloud→google` sem Premium (**gate runtime**) e resolve o descritor de orçamento (`{scope,key,seats?}`, precedência SEM spill: Plus→pool pessoal, senão passe, senão guild-direto) que viaja no `SynthRequest.gcloudBudget` até ao chokepoint. Cobertura total do gate por um só ponto. | `store/premium`, `store/userVoice` |
...
| `tts/deCaps.ts` | `lowerAllCapsRuns(text)` (puro): baixa corridas de 2+ MAIÚSCULAS para o motor **não SOLETRAR o "grito"** (lê a palavra). Usado pelo gTTS (`deCapsForGoogle`) e aplicado também no Kokoro/Clone/Neural; o Piper é no-op (lê maiúsculas como palavra). O ganho de volume do grito é à parte (`emphasis.ts`, no player). | — |
| `tts/emphasis.ts` | `emphasisGain(text)` (puro): multiplicador de VOLUME (grito) a partir de `!`/MAIÚSCULAS, aplicado na reprodução (`player.ts`) — engine- e língua-agnóstico. Calculado do `emphasisSource` (palavras do user), não do texto decorado. | — |
| `tts/prosody.ts` | `ProsodyEngine` (decorator): **entoação de pergunta** — quando a fala acaba em `?`, corta os últimos ~500 ms e sobe-lhes o tom via ffmpeg (`asetrate`+`atempo`, só filtros CORE), reconcatenando. Cache própria `q`; formato ≠ 22050/mono/16 ou qualquer erro → voz limpa (**nunca lança**). `isQuestion`/`splitTailWav` puros. Fica na pilha de decoradores entre o `CloneEngine` e o `EffectEngine`. | `effects`, `wavConcat`, `cache` |
