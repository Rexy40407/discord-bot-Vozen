...
> **To delete everything at once, use `/privacy erase`** — it asks you to confirm, then removes all data tied to your user ID across every server (`user_voice`, `user_nickname`, `user_abbreviation`, `pronunciation_user`, `user_effect`, `tts_optout`, `stt_consent`, `game_score`, `talk_stats`, `user_birthday`). (Transcription text already posted to a channel is a normal Discord message, not bot-side data — see section 2.3.) Active entitlements and the minimum purchase records needed for accounting, fraud prevention, disputes, and legal obligations may be retained despite an erasure request. They are restricted to those purposes and deleted or anonymized when no longer necessary; contact the operator to cancel Premium or exercise a legal right.
