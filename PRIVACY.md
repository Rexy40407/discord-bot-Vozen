...
| **Voice clone**               | `user_clone`              | `user_id`, `sample_path`, `consent_at`, `enabled`, `target_id`                                                                                                                                                                                                              | **See section 2.3 — the most sensitive data.** Points to the sample's `.wav` file + the consent timestamp. `target_id` is the Discord ID of the person whose **voice was recorded** (equal to `user_id` for a self-clone; different when someone recorded another person with their consent) — that person can always revoke it. |
...
> **To delete everything at once, use `/privacy erase`** — it asks you to confirm, then removes all data tied to your user ID across every server (`user_voice`, `user_nickname`, `user_abbreviation`, `pronunciation_user`, `user_effect`, `tts_optout`, `user_clone` including its `.wav`, `stt_consent`, `game_score`, `talk_stats`, `user_birthday`). It also revokes any clone made from **your** voice by other people. (Transcription text already posted to a channel is a normal Discord message, not bot-side data — see section 2.4.) Active entitlements and the minimum purchase records needed for accounting, fraud prevention, disputes, and legal obligations may be retained despite an erasure request. They are restricted to those purposes and deleted or anonymized when no longer necessary; contact the operator to cancel Premium or exercise a legal right.
...
- **Operator admin console (opt-in).** The operator may enable a private admin console (the `/api/admin/*` routes, gated by password **and** the operator's own Discord login) to grant/revoke Premium and to view **aggregate usage per server**: the server's name and icon (from Discord), how many messages Vozen has read there, and the top talkers — the same "chatterboxes" data already shown publicly by `/topspeakers` in each server. **No new data is collected for this**: it only surfaces, to the operator, data the bot already stores and already discloses, and every deletion path is unchanged (`/privacy erase`, and the automatic purge of data for servers the bot has left). Inert unless the `ADMIN_*` env vars are configured.
...
_Vozen is a self-hosted, open-source Discord TTS bot (AGPL-3.0). The instance operator is the data controller. The bot stores Discord numeric IDs plus the preferences and service records described above. Message text is processed transiently and is not stored in a database table; generated audio is kept only in a bounded, regenerable cache. Where text goes depends on `TTS_ENGINE`: Piper is local; legacy gTTS/router modes may send text to Google; neural sends text to OpenAI; Premium Google HD uses the official Google Cloud TTS API. Voice cloning and transcription are consent-first and local on the configured instance. Vozen does not sell data, run third-party advertising analytics, or use message content for model training. Use `/privacy erase` to delete your data, or contact the [support server](https://discord.gg/V6PZYZmhcQ) to request access to a copy of it or exercise any other data right._
