...
Attachment-message transcription is not enabled by this release: Vozen will not download or decode
an arbitrary attachment until it can enforce trusted-host, byte, duration and timeout bounds end to
end. The existing live `/transcribe` consent rules remain unchanged.
...
Text translation is **off by default**. A server administrator must explicitly configure a
...
channel. It does not translate DMs, private threads, attachments, embeds, message history, voice
...
Mappings, an optional member opt-out and bounded daily character counters are stored
...
is never shown to ordinary members. Translation is text-only and **never automatically speaks** a
translation or bypasses Vozen's separate same-call rule for voice.
