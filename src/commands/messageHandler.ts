...
/**
 * Autojoin: quando o Vozi ainda não está numa call e o autor está num canal de voz,
...
export async function handleMessage(message: Message, deps: BotDeps): Promise<void> {
  try {
    ...
    const me = deps.client.user;
    if (!me) return;
    // O Vozi NUNCA se lê a si próprio — anti-loop, independentemente do read_bots.
    ...
    // Vozi está agora (o texto do canal de voz tem channelId == id do canal de voz).
    ...
    // gating: jogador ativo nesta guild. Com autojoin ON, se o Vozi ainda não está
    ...
    // texto REALMENTE falado (req.text + segmentos) — o Vozi lê a mensagem SEM dizer
