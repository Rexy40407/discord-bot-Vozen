export default {
  ...
  'privacy.eraseConfirm':
    "⚠️ Це назавжди видаляє **всі** ваші дані Vozen на кожному сервері: налаштування голосу, вимовлюване ім'я, особисті скорочення та вимови, збережений день народження, ігрові бали, статистику розмов, відмову від читання та будь-який клон голосу (зокрема записи вашого голосу, зроблені іншими). **Це не можна скасувати.** Ви впевнені?",
  ...
  'clone.locked':
    '🔒 Клонування голосу — це функція Premium (воно потребує реальних обчислень). Перегляньте `/premium`.',
  'clone.notInVoice':
    'Щоб записувати, вам потрібно бути в голосовому каналі **разом зі мною**. Спершу скористайтеся `/join`.',
  'clone.alreadyRecording':
    'Ви вже записуєте зразок — завершіть його (або натисніть **⏹️ Стоп**), перш ніж починати інший.',
  'clone.recording':
    '🎙️ **Записую ваш голос** — говоріть, доки запис не зупиниться сам (~{target}с мовлення, паузи не рахуються), або натисніть **⏹️ Стоп**, коли завершите. Я зберігаю лише ВАШ звук.',
  'clone.recordingOther':
    '🎙️ **Записую {who}** — нехай говорить, доки запис не зупиниться сам (~{target}с мовлення, паузи не рахуються), або натисніть **⏹️ Стоп**, щоб завершити.',
  'clone.recordingProgress': '🔴 Записую… захоплено **{got}с / {target}с** мовлення. Продовжуйте!',
  'clone.consentRequest':
    '🎙️ {invoker} хоче записати **ваш голос** ({target}с мовлення), щоб створити клон голосу, яким зможе говорити. Дозволяєте? *(спливає за 60с)*',
  'clone.consentAllow': 'Дозволити',
  'clone.consentDeny': 'Ні',
  'clone.consentNotYou': 'Відповісти на це може лише та людина, яку записують.',
  'clone.consentGranted': '✅ {who} погодився — починаю запис.',
  'clone.consentRefused': '✖️ {who} відмовився. Запис скасовано — жодного звуку не захоплено.',
  'clone.consentTimeout': '⌛ {who} не відповів вчасно. Запис скасовано.',
  'clone.consentWaiting': '⏳ Чекаю, поки {who} погодиться в каналі…',
  'clone.targetNotInVoice':
    '{who} має бути в голосовому каналі **разом зі мною**, щоб його записати. Попросіть його спершу виконати `/join`.',
  'clone.pickFromList':
    'Оберіть людину зі списку пропозицій (записати можна лише тих, хто в дзвінку). Залиште порожнім, щоб записати себе.',
  'clone.stopBtn': 'Стоп',
  'clone.stopNotYours': 'Зупинити запис може лише той, хто його веде.',
  'clone.tooShort':
    'Я вловив лише {seconds}с мовлення — для якісного клону потрібно щонайменше ~{min}с (ціль була {target}с). Спробуйте ще раз за допомогою `/voice clone record`.',
  'clone.saved':
    '✅ Зразок голосу збережено ({seconds}с мовлення). Увімкніть його за допомогою `/voice clone use active:true`. Використовувати ваш клон можете лише ВИ; видаліть його будь-коли за допомогою `/voice clone delete`.',
  'clone.savedOther':
    '✅ Збережено {seconds}с голосу {who} як ВАШ клон. Увімкніть його за допомогою `/voice clone use active:true`; видаліть будь-коли за допомогою `/voice clone delete`.',
  'clone.failed':
    'Запис не вдався — спробуйте ще раз. Якщо це повторюється, перезайдіть до голосового каналу.',
  'clone.none':
    'У вас ще немає клону голосу. Запишіть його за допомогою `/voice clone record` (Premium).',
  'clone.deleted':
    '🗑️ Клон голосу видалено — зразок і запис згоди прибрано, жодного сліду не залишилося.',
  'clone.revoked':
    '🛑 Згоду відкликано — видалено {count} клон(ів) голосу, які інші люди зробили з вашого голосу.',
  'clone.status': '🧬 Клон голосу: зразок записано {date} · зараз **{state}**.',
  'clone.stateOn': 'УВІМКНЕНО',
  'clone.stateOff': 'вимкнено',
  'clone.noSample':
    'Спершу вам потрібен зразок — запишіть його за допомогою `/voice clone record`.',
  'clone.enabled':
    '✅ Тепер ваші повідомлення читатимуться **вашим клонованим голосом**. Вимкніть будь-коли за допомогою `/voice clone use active:false`.',
  'clone.enabledNoEngine':
    '✅ Збережено — але рушій клонування ще не встановлено на цьому екземплярі, тож поки що ви чутимете звичайний голос.',
  'clone.disabled': '✅ Клонований голос вимкнено — назад до вашого звичайного голосу.',
  'voice.effect.locked':
  ...
  'premium.getHint':
    'Усе, чим ви користуєтеся сьогодні, лишається безкоштовним. Premium додає всі 8 голосових ефектів, клонування голосу, режим 24/7 у дзвінку, 50 особистих вимов, /rizz та преміум-ігри. Підтримка: https://ko-fi.com/',
  'premium.linePass':
    '🎟️ **Ваш пропуск Premium:** використано {used}/{total} ліцензій · спливає {date}',
  'premium.passServers': '↳ Використовується на: {servers}',
  'premium.pitch':
    'У вас ще немає Premium. **Vozen Premium** (€3.99/міс за 3 сервери або €7.99/міс за 8) розблоковує для всього сервера: усі 8 голосових ефектів, клонування голосу, режим 24/7 у дзвінку, 50 особистих вимов (замість 3), команду /rizz та преміум-ігри (Ланцюжок слів, Wordle, Шахи). **Vozen Plus** (€1.99/міс) дає вам ці переваги особисто, на будь-якому сервері.',
  'premium.buyHint':
  ...
  'stt.busyClone':
    '⏳ Просто зараз хтось записує голосовий клон у цьому дзвінку. У мене лише один мікрофон — спробуй ще раз, коли завершиться (кілька секунд).',
  'clone.busyStt':
    '⏳ У цьому дзвінку працює транскрипція, а в мене лише один мікрофон. Спершу виконай `/transcribe stop`, а потім записуй свій клон.',
};
