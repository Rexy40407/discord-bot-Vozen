export default {
  ...
  'privacy.eraseConfirm':
    '⚠️ Бұл барлық серверлердегі **барлық** Vozen деректеріңізді біржола жояды: дауыс параметрлері, айтылатын лақап ат, жеке қысқартулар мен айтылымдар, сақталған туған күн, ойын ұпайлары, сөйлеу статистикасы, бас тарту, және кез келген дауыс клоны (басқалар жасаған дауысыңыздың жазбаларын қоса). **Мұны қайтару мүмкін емес.** Сенімдісіз бе?',
  ...
  'clone.locked':
    '🔒 Дауысты клондау — Premium мүмкіндігі (нақты есептеу ресурсын қажет етеді). `/premium` қараңыз.',
  'clone.notInVoice':
    'Жазу үшін **менімен бірге** дауыстық арнада болуыңыз керек. Алдымен `/join` қолданыңыз.',
  'clone.alreadyRecording':
    'Сіз әлдеқашан үлгі жазып жатырсыз — жаңасын бастамас бұрын оны аяқтаңыз (немесе **⏹️ Stop** басыңыз).',
  'clone.recording':
    '🎙️ **Дауысыңыз жазылуда** — өздігінен тоқтағанша сөйлей беріңіз (~{target}с сөйлеу, кідірістер есептелмейді), немесе біткен кезде **⏹️ Stop** басыңыз. Мен тек СІЗДІҢ аудионы сақтаймын.',
  'clone.recordingOther':
    '🎙️ **{who} жазылуда** — өздігінен тоқтағанша сөйлей беруі керек (~{target}с сөйлеу, кідірістер есептелмейді), немесе аяқтау үшін **⏹️ Stop** басыңыз.',
  'clone.recordingProgress':
    '🔴 Жазылуда… **{got}с / {target}с** сөйлеу түсірілді. Жалғастыра беріңіз!',
  'clone.consentRequest':
    '🎙️ {invoker} сөйлей алатын дауыс клонын жасау үшін **сіздің дауысыңызды** ({target}с сөйлеу) жазғысы келеді. Рұқсат етесіз бе? *(60с ішінде мерзімі бітеді)*',
  'clone.consentAllow': 'Рұқсат ету',
  'clone.consentDeny': 'Жоқ',
  'clone.consentNotYou': 'Бұған тек жазылып жатқан адам ғана жауап бере алады.',
  'clone.consentGranted': '✅ {who} келісті — жазу басталуда.',
  'clone.consentRefused': '✖️ {who} бас тартты. Жазу болдырылмады — ешқандай аудио түсірілген жоқ.',
  'clone.consentTimeout': '⌛ {who} уақтылы жауап бермеді. Жазу болдырылмады.',
  'clone.consentWaiting': '⏳ {who} арнада қабылдауын күтудеміз…',
  'clone.targetNotInVoice':
    'Жазылу үшін {who} **менімен бірге** дауыстық арнада болуы керек. Алдымен оған `/join` орындауды өтініңіз.',
  'clone.pickFromList':
    'Ұсыныстар тізімінен адамды таңдаңыз (тек қоңыраудағы адамдарды ғана жазуға болады). Өзіңізді жазу үшін бос қалдырыңыз.',
  'clone.stopBtn': 'Тоқтату',
  'clone.stopNotYours': 'Оны тек жазып жатқан адам ғана тоқтата алады.',
  'clone.tooShort':
    'Мен тек {seconds}с сөйлеу ғана түсірдім — жақсы клондау үшін кемінде ~{min}с қажет (мақсат {target}с болды). `/voice clone record` арқылы қайталап көріңіз.',
  'clone.saved':
    '✅ Дауыс үлгісі сақталды ({seconds}с сөйлеу). Оны `/voice clone use active:true` арқылы қосыңыз. Клоныңызды тек СІЗ ғана қолдана аласыз; кез келген уақытта `/voice clone delete` арқылы жойыңыз.',
  'clone.savedOther':
    '✅ {who} дауысының {seconds}с СІЗДІҢ клоныңыз ретінде сақталды. Оны `/voice clone use active:true` арқылы қосыңыз; кез келген уақытта `/voice clone delete` арқылы жойыңыз.',
  'clone.failed':
    'Жазу сәтсіз аяқталды — қайталап көріңіз. Егер бұл қайталана берсе, дауыстық арнаға қайта кіріңіз.',
  'clone.none': 'Сізде әлі дауыс клоны жоқ. `/voice clone record` арқылы жазыңыз (Premium).',
  'clone.deleted':
    '🗑️ Дауыс клоны жойылды — үлгі мен келісім жазбасы жойылды, ешқандай із қалмады.',
  'clone.revoked':
    '🛑 Келісім қайтарылды — басқалар сіздің дауысыңыздан жасаған {count} дауыс клоны жойылды.',
  'clone.status': '🧬 Дауыс клоны: үлгі {date} жазылды · қазір **{state}**.',
  'clone.stateOn': 'ҚОСУЛЫ',
  'clone.stateOff': 'өшірулі',
  'clone.noSample': 'Алдымен сізге үлгі керек — `/voice clone record` арқылы жазыңыз.',
  'clone.enabled':
    '✅ Хабарламаларыңыз енді **клондалған дауысыңызбен** оқылады. Кез келген уақытта `/voice clone use active:false` арқылы өшіріңіз.',
  'clone.enabledNoEngine':
    '✅ Сақталды — бірақ клон қозғалтқышы осы данада әлі орнатылмаған, сондықтан әзірге қалыпты дауысты естисіз.',
  'clone.disabled': '✅ Клондалған дауыс өшірулі — қалыпты дауысыңызға оралдыңыз.',
  'voice.effect.locked':
  ...
  'premium.getHint':
    'Бүгін қолданатын барлық нәрсе тегін болып қала береді. Premium 8 дауыс эффектісінің барлығын, дауысты клондауды, қоңырауда 24/7 болуды, 50 жеке айтылымды, /rizz-ті және премиум ойындарды қосады. Қолдау: https://ko-fi.com/',
  'premium.linePass':
    '🎟️ **Сіздің Premium рұқсатнамаңыз:** {used}/{total} лицензия қолданыста · {date} мерзімі бітеді',
  'premium.passServers': '↳ Қолданыста: {servers}',
  'premium.pitch':
    'Сізде әлі Premium жоқ. **Vozen Premium** (3 сервер үшін €3.99/ай немесе 8 сервер үшін €7.99/ай) бүкіл сервер үшін ашады: 8 дауыс эффектісінің барлығын, дауысты клондауды, қоңырауда 24/7 болуды, 50 жеке айтылымды (3-тің орнына), /rizz командасын және премиум ойындарды (Сөз тізбегі, Wordle, Шахмат). **Vozen Plus** (€1.99/ай) осы артықшылықтарды кез келген серверде жеке өзіңізге береді.',
  'premium.buyHint':
  ...
  'stt.busyClone':
    '⏳ Дәл қазір осы қоңырауда біреу дауыс клонын жазып жатыр. Менде тек бір микрофон бар — аяқталған соң қайта көріңіз (бірнеше секунд).',
  'clone.busyStt':
    '⏳ Бұл қоңырауда транскрипция жұмыс істеп тұр, ал менде тек бір микрофон бар. Алдымен `/transcribe stop` командасын орындаңыз, сосын клоныңызды жазыңыз.',
};
