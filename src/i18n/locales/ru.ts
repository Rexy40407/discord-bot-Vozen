export default {
  ...
  'privacy.eraseConfirm':
    '⚠️ Это навсегда удалит **все** ваши данные Vozen на всех серверах: настройки голоса, произносимое имя, личные сокращения и произношения, сохранённый день рождения, игровые очки, статистику речи, отказ от чтения и любой клон голоса (включая записи вашего голоса, сделанные другими). **Это нельзя отменить.** Вы уверены?',
  ...
  'clone.locked':
    '🔒 Клонирование голоса — это функция Premium (оно требует реальных вычислений). См. `/premium`.',
  'clone.notInVoice':
    'Чтобы записывать, вам нужно быть в голосовом канале **вместе со мной**. Сначала используйте `/join`.',
  'clone.alreadyRecording':
    'Вы уже записываете образец — завершите его (или нажмите **⏹️ Стоп**), прежде чем начинать новый.',
  'clone.recording':
    '🎙️ **Записываю ваш голос** — продолжайте говорить, пока запись не остановится сама (~{target}с речи, паузы не считаются), или нажмите **⏹️ Стоп**, когда закончите. Я храню только ВАШ звук.',
  'clone.recordingOther':
    '🎙️ **Записываю {who}** — ему(ей) нужно продолжать говорить, пока запись не остановится сама (~{target}с речи, паузы не считаются), или нажать **⏹️ Стоп**, чтобы завершить.',
  'clone.recordingProgress': '🔴 Записываю… захвачено **{got}с / {target}с** речи. Продолжайте!',
  'clone.consentRequest':
    '🎙️ {invoker} хочет записать **ваш голос** ({target}с речи), чтобы создать клон голоса, которым сможет говорить. Разрешаете? *(истекает через 60с)*',
  'clone.consentAllow': 'Разрешить',
  'clone.consentDeny': 'Нет',
  'clone.consentNotYou': 'Ответить на это может только тот, кого записывают.',
  'clone.consentGranted': '✅ {who} согласился — начинаю запись.',
  'clone.consentRefused': '✖️ {who} отказался. Запись отменена — звук не захвачен.',
  'clone.consentTimeout': '⌛ {who} не ответил вовремя. Запись отменена.',
  'clone.consentWaiting': '⏳ Жду, пока {who} согласится в канале…',
  'clone.targetNotInVoice':
    'Чтобы записать {who}, ему(ей) нужно быть в голосовом канале **вместе со мной**. Попросите его(её) сначала запустить `/join`.',
  'clone.pickFromList':
    'Выберите человека из списка подсказок (записать можно только тех, кто в звонке). Оставьте пустым, чтобы записать себя.',
  'clone.stopBtn': 'Стоп',
  'clone.stopNotYours': 'Остановить запись может только тот, кто её ведёт.',
  'clone.tooShort':
    'Я захватил всего {seconds}с речи — для качественного клона нужно хотя бы ~{min}с (цель была {target}с). Попробуйте ещё раз через `/voice clone record`.',
  'clone.saved':
    '✅ Образец голоса сохранён ({seconds}с речи). Включите его через `/voice clone use active:true`. Использовать ваш клон можете только ВЫ; удалить его можно в любой момент через `/voice clone delete`.',
  'clone.savedOther':
    '✅ Сохранил {seconds}с голоса {who} как ВАШ клон. Включите его через `/voice clone use active:true`; удалить можно в любой момент через `/voice clone delete`.',
  'clone.failed':
    'Запись не удалась — попробуйте ещё раз. Если это повторяется, перезайдите в голосовой канал.',
  'clone.none': 'У вас пока нет клона голоса. Запишите его через `/voice clone record` (Premium).',
  'clone.deleted': '🗑️ Клон голоса удалён — образец и запись согласия удалены, следов не осталось.',
  'clone.revoked':
    '🛑 Согласие отозвано — удалил {count} клон(ов) голоса, которые другие люди сделали из вашего голоса.',
  'clone.status': '🧬 Клон голоса: образец записан {date} · сейчас **{state}**.',
  'clone.stateOn': 'ВКЛ',
  'clone.stateOff': 'выкл',
  'clone.noSample': 'Сначала нужен образец — запишите его через `/voice clone record`.',
  'clone.enabled':
    '✅ Теперь ваши сообщения будут читаться **вашим клонированным голосом**. Отключить можно в любой момент через `/voice clone use active:false`.',
  'clone.enabledNoEngine':
    '✅ Сохранено — но движок клонирования пока не установлен на этом экземпляре, поэтому пока вы будете слышать обычный голос.',
  'clone.disabled': '✅ Клонированный голос выключен — снова ваш обычный голос.',
  ...
  'premium.getHint':
    'Всё, чем вы пользуетесь сегодня, остаётся бесплатным. Premium добавляет все 8 голосовых эффектов, клонирование голоса, режим 24/7 в звонке, 50 личных произношений, /rizz и премиум-игры. Поддержать: https://ko-fi.com/',
  'premium.linePass':
    '🎟️ **Ваш пропуск Premium:** используется лицензий {used}/{total} · истекает {date}',
  'premium.passServers': '↳ Используется на: {servers}',
  'premium.pitch':
    'У вас пока нет Premium. **Vozen Premium** (€3.99/мес за 3 сервера или €7.99/мес за 8) разблокирует для всего сервера: все 8 голосовых эффектов, клонирование голоса, режим 24/7 в звонке, 50 личных произношений (вместо 3), команду /rizz и премиум-игры (Цепочка слов, Wordle, Шахматы). **Vozen Plus** (€1.99/мес) даёт вам эти бонусы лично, на любом сервере.',
  'premium.buyHint':
  ...
  'vote.upsell':
    '🗳️ Нет Plus? Проголосуйте за Vozen на top.gg → **24 ч Plus бесплатно** (раз в месяц): {url}',
  'vote.cooldownStatus':
    '🗳️ Вы уже получили награду за голос — проголосуйте снова ради ещё **24 ч Plus** {date}.',
  ...
  'stt.busyClone':
    '⏳ Прямо сейчас кто-то записывает голосовой клон в этом звонке. У меня только один микрофон — попробуй ещё раз, когда закончится (несколько секунд).',
  'clone.busyStt':
    '⏳ В этом звонке идёт транскрипция, а у меня только один микрофон. Сначала выполни `/transcribe stop`, потом записывай свой клон.',
};
