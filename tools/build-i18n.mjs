...
const OUTPUT = join(ROOT, 'site', 'js', 'i18n-v37.js');
...
if (checkOnly) {
  ...
  if (current !== out) {
    throw new Error('site/js/i18n-v37.js is out of date; run npm run build:i18n');
  }
  console.log(
  ...
  console.log(
    `[build-i18n] ${LANGS.length} languages · ${uiKeys.length} UI keys · ${FAQ.length} FAQ -> site/js/i18n-v37.js`,
  );
}
